...
class TestTendersTest(object):
    ...
    def test10_Compare_tender_id(self):
        assert json_cdb['data']['tenderID'] == BrokerBasedFunctions(self.broker).get_tender_uid()
    ...
    def test11_Compare_tender_title(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['title'] == BrokerBasedFunctions(self.broker).get_tender_title()

    def test12_Compare_tender_description(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['description'] == json_cdb['data']['description']
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['description'] == BrokerBasedFunctions(self.broker).get_tender_description()

    def test13_compare_tender_value_amount(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['value']['amount'] == json_cdb['data']['value']['amount']
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['value']['amount'] == BrokerBasedFunctions(self.broker).get_tender_value_amount()

    def test14_Compare_tender_currency(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['value']['currency'] == json_cdb['data']['value']['currency']
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['value']['currency'] == BrokerBasedFunctions(self.broker).get_tender_currency()
    ...
    def test15_compare_value_tax_included(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['value']['valueAddedTaxIncluded'] == BrokerBasedFunctions(self.broker).get_value_added_tax_included()
    ...
    def test16_Compare_owner_country(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['address']['countryName'] == BrokerBasedFunctions(self.broker).get_owner_country()
    ...
    def test17_Compare_owner_locality(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['address']['locality'] == BrokerBasedFunctions(self.broker).get_owner_locality()
    ...
    def test18_Compare_owner_postal_code(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['address']['postalCode'] == BrokerBasedFunctions(self.broker).get_owner_postal_code()
    ...
    def test19_Compare_owner_region(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['address']['region'] == BrokerBasedFunctions(self.broker).get_owner_region()
    ...
    def test20_Compare_owner_street(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['address']['streetAddress'] == BrokerBasedFunctions(self.broker).get_owner_street()
    ...
    def test21_Compare_owner_contact_name(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['contactPoint']['name'] == BrokerBasedFunctions(self.broker).get_owner_contact_name()
    ...
    def test22_Compare_owner_phone_number(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['contactPoint']['telephone'] == BrokerBasedFunctions(self.broker).get_owner_phone_number()
    ...
    def test23_Compare_owner_site(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['contactPoint']['url'] == BrokerBasedFunctions(self.broker).get_owner_site()

    def test24_Compare_owner_company_name(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['name'] == json_cdb['data']['procuringEntity']['name']
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['name'] == BrokerBasedFunctions(self.broker).get_owner_company_name()
    ...
    def test25_Compare_owner_identifier(self):
        ...
        with pytest.allure.step(msg.compare_site):
            assert self.pmt['data']['procuringEntity']['identifier']['id'] == BrokerBasedFunctions(self.broker).get_owner_identifier()

    def test26_Compare_item_description(self):
        with pytest.allure.step(msg.compare_cdb):
            compare_item_description_cdb(self.pmt, json_cdb['data']['id'])
        with pytest.allure.step(msg.compare_site):
            BrokerBasedFunctions(self.broker).compare_item_description_page(self.pmt)
