...
class TestTendersTest(object):
    ...
    def test2_add_documents_tender(self):
        BrokerBasedActions(self.broker).open_tender_edit_page(DATA['json_cdb']['data']['tenderID'])
        # global docs_data
    ...
    def test14_compare_tender_currency(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['value']['currency'] == DATA['json_cdb']['data']['value']['currency']
        with pytest.allure.step(msg.compare_site):
    ...
    def test15_compare_value_tax_included(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['value']['valueAddedTaxIncluded'] == DATA['json_cdb']['data']['value']['valueAddedTaxIncluded']
        with pytest.allure.step(msg.compare_site):
    ...
    def test16_compare_owner_country(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['address']['countryName'] == DATA['json_cdb']['data']['procuringEntity']['address']['countryName']
        with pytest.allure.step(msg.compare_site):
    ...
    def test17_compare_owner_locality(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['address']['locality'] == DATA['json_cdb']['data']['procuringEntity']['address']['locality']
        with pytest.allure.step(msg.compare_site):
    ...
    def test18_compare_owner_postal_code(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['address']['postalCode'] == DATA['json_cdb']['data']['procuringEntity']['address']['postalCode']
        with pytest.allure.step(msg.compare_site):
    ...
    def test19_compare_owner_region(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['address']['region'] == DATA['json_cdb']['data']['procuringEntity']['address']['region']
        with pytest.allure.step(msg.compare_site):
    ...
    def test20_compare_owner_street(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['address']['streetAddress'] == DATA['json_cdb']['data']['procuringEntity']['address']['streetAddress']
        with pytest.allure.step(msg.compare_site):
    ...
    def test21_compare_owner_contact_name(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['contactPoint']['name'] == DATA['json_cdb']['data']['procuringEntity']['contactPoint']['name']
        with pytest.allure.step(msg.compare_site):
    ...
    def test22_compare_owner_phone_number(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['contactPoint']['telephone'] == DATA['json_cdb']['data']['procuringEntity']['contactPoint']['telephone']
        with pytest.allure.step(msg.compare_site):
    ...
    def test23_compare_owner_site(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['contactPoint']['url'] == DATA['json_cdb']['data']['procuringEntity']['contactPoint']['url']
        with pytest.allure.step(msg.compare_site):
    ...
    def test24_compare_owner_company_name(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['name'] == DATA['json_cdb']['data']['procuringEntity']['name']
        with pytest.allure.step(msg.compare_site):
    ...
    def test25_compare_owner_identifier(self):
        with pytest.allure.step(msg.compare_cdb):
            assert self.pmt['data']['procuringEntity']['identifier']['id'] == DATA['json_cdb']['data']['procuringEntity']['identifier']['id']
        with pytest.allure.step(msg.compare_site):
    ...
    def test26_compare_item_description(self):
        with pytest.allure.step(msg.compare_cdb):
            compare_item_description_cdb(self.pmt, DATA['json_cdb']['data']['id'])
        with pytest.allure.step(msg.compare_site):
